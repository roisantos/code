...
def calc_result(np_pred:np.ndarray, np_label:np.ndarray, thresh_value=None):
    ...
    uni_label = np.unique(np_label)
    uni_pred = np.unique(np_pred)
    # print(f"Unique label values: {uni_label}")
    # print(f"Unique pred values: {uni_pred}")

    # print(uni)
    assert (len(uni_label) == 2) and (1 in uni_label) and (0 in uni_label), "Label array does not contain both 0 and 1"

...
def train_and_evaluate(model_name, dataset, config, logging_enabled=False):
    ...
    num_workers = training_config.get("num_workers", 16)
    ...
    valLoader = DataLoader(dataset=dataset['val'], batch_size=batch_size, shuffle=True, collate_fn=custom_collate, num_workers=num_workers, pin_memory=True)
    testLoader = DataLoader(dataset=dataset['test'], batch_size=batch_size, shuffle=False, collate_fn=custom_collate, num_workers=num_workers, pin_memory=True)
    ...
    for epoch in range(epochs):
        ...
        result_train = traverseDataset(
            ...
            description=f"Train Epoch {epoch}", device=device, funcLoss=funcLoss, optimizer=optimizer, accumulation_steps=accumulation_steps, scaler=scaler
        )
        ...
        result_val = traverseDataset(
            ...
            description=f"Val Epoch {epoch}", device=device, funcLoss=funcLoss, accumulation_steps=accumulation_steps, scaler=scaler
        )
        ...
        if dice > bestResult['dice']:
            ...
            result_test = traverseDataset(
                ...
                log_section=None, log_writer=None, description=f"Test Epoch {epoch}", device=device, funcLoss=funcLoss, accumulation_steps=accumulation_steps, scaler=scaler
            )
